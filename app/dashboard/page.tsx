...
const MapComponent = dynamic(() => import('@/components/MapComponent'), { 
  ssr: false,
  loading: () => <div className="w-full h-full bg-zinc-900 animate-pulse flex items-center justify-center text-zinc-500 font-mono text-sm">INITIALIZING MAP...</div>
});
...
export default function DashboardPage() {
  ...
  const criticalCount = activeCalls.filter(c => c.severity === 'critical').length;
        ...
        <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-6 bg-zinc-950/50 backdrop-blur-sm z-10">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900 rounded-full border border-zinc-800">
              ...
              <span className="text-xs font-mono text-zinc-400">MESH NETWORK ONLINE</span>
            </div>
            {criticalCount > 0 && (
              ...
              {loading ? (
                ...
                activeCalls.map(call => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${
                          call.severity === 'critical' ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' :
                          call.severity === 'high' ? 'bg-orange-500' :
                          'bg-yellow-500'
                        }`} />
                        <span className="font-mono text-xs text-zinc-400">{call.node}</span>
                      </div>
                    ...
                    <p className="text-sm font-medium text-zinc-200">{call.message}</p>
                    <div className="mt-3 flex items-center justify-between">
                      <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded ${
                        call.severity === 'critical' ? 'bg-red-500/10 text-red-500' :
                        call.severity === 'high' ? 'bg-orange-500/10 text-orange-500' :
